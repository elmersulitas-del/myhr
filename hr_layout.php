  ...
  <div class="min-h-screen flex">
    ...
    <aside class="hidden md:flex md:w-64 flex-col border-r bg-white">
      <div class="px-6 py-5 border-b">
        <p class="text-lg font-extrabold">HR Panel</p>
        <p class="text-xs text-slate-500">System Administration</p>
      </div>

      <nav class="flex-1 px-3 py-4 space-y-1">
        <a href="dashboard_hr.php" class="<?php echo nav_class('home', $active); ?>">
          <span>🏠</span> Home
        </a>

        <a href="hr_approvals.php"
           class="flex items-center justify-between rounded-xl px-3 py-2 text-sm font-semibold hover:bg-slate-100 <?php echo $active==='approvals' ? 'bg-slate-900 text-white hover:bg-slate-900' : ''; ?>">
          <span class="flex items-center gap-3"><span>✅</span> HR Approvals</span>
          <span class="<?php echo ($pendingHr ?? 0) > 0 ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'; ?> text-xs font-bold px-2 py-1 rounded-full">
            <?php echo (int)($pendingHr ?? 0); ?>
          </span>
        </a>

        <a href="employees.php" class="<?php echo nav_class('employees', $active); ?>">
          <span>👥</span> Employee Records
        </a>

        <a href="leave_admin.php" class="<?php echo nav_class('leave', $active); ?>">
          <span>🗂️</span> Leave Management
        </a>

        

        <a href="announcements.php" class="<?php echo nav_class('announcements', $active); ?>">
          <span>📢</span> Announcements
        </a>

        <a href="calendar.php" class="<?php echo nav_class('calendar', $active); ?>">
          <span>📅</span> Calendar
        </a>

        <a href="attendance_admin.php" class="<?php echo nav_class('attendance', $active); ?>">
